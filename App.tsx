...
const App: React.FC = () => {
  ...
  const [state, setState] = useState<AppState>({
    ...
    settings: {
        ...
        qrCodeUrl: '',
        paymentInstructions: '',
        supportEmail: 'Nadimanwar794@gmail.com',
        footerText: '',
        showFooter: true,
        footerColor: '',
        packages: [
      ...
      const unsubscribe = subscribeToSettings((newSettings) => {
          if (newSettings) {
              setState(prev => {
                  ...
                  // Simple change detection (may have order false positives but ensures updates aren't missed)
  ...
  useEffect(() => {
    ...
    if (loggedInUserStr) {
      try {
        ...
        // No onboarding — auto-fill missing fields silently
        if (user.role === 'STUDENT' || user.role === 'TEACHER') {
            if (!user.profileCompleted) user.profileCompleted = true;
            if (!user.board) user.board = 'CBSE';
            if (!user.classLevel) user.classLevel = '6';
        }
  ...
  useEffect(() => {
    ...
    // Skip onboarding entirely — auto-fill any missing profile fields with sensible defaults
    if (user.role === 'STUDENT' || user.role === 'TEACHER') {
        if (!user.profileCompleted) user.profileCompleted = true;
        if (!user.board) user.board = 'CBSE';
        if (!user.classLevel) user.classLevel = '6';
  ...
  if (isAppLoading) {
      return <AppLoadingScreen onComplete={() => setIsAppLoading(false)} />;
  }
