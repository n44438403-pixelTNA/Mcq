...
const RevisionHubComponent: React.FC<Props> = ({ user, onTabChange, settings, onNavigateContent, onUpdateUser }) => {
    ...
    const pendingNotes = useMemo(() => {
        // Include WEAK/AVERAGE topics immediately so they show up right after an MCQ attempt,
        // plus any topic whose scheduled nextRevision date has arrived.
        return topics.filter(t => {
            const dueByDate = !!(t.nextRevision && new Date(t.nextRevision) <= now);
            const needsAttention = t.status === 'WEAK' || t.status === 'AVERAGE';
            return dueByDate || needsAttention;
        });
    }, [topics]);

    const pendingMcqs = useMemo(() => {
        // Include WEAK/AVERAGE topics immediately so freshly attempted chapters surface for re-test.
        return topics.filter(t => {
            const dueByDate = !!(t.mcqDueDate && new Date(t.mcqDueDate) <= now);
            const needsAttention = t.status === 'WEAK' || t.status === 'AVERAGE';
            return dueByDate || needsAttention;
        });
    }, [topics]);
                        ...
                        if (subTopics.size > 0) {
                            ...
                            subTopics.forEach(subName => {
                                ...
                                if (data.manualMcqData) {
                                    subNotesCount += data.manualMcqData.filter((q: any) => q.topic && q.topic.trim() === subName && (q.quickNote || q.deepNote)).length;
                                }
                            ...
                            let totalNotes = 0;
                            if (data.topicNotes) totalNotes += data.topicNotes.length;
                            if (data.manualMcqData) totalNotes += data.manualMcqData.filter((q: any) => q.quickNote || q.deepNote).length;
                            topic.notesCount = totalNotes;
                        }
                    }
                } catch (e) {
            ...
            <div className="bg-white p-5 rounded-3xl border border-slate-100 flex items-center gap-4 shadow-md relative overflow-hidden z-10 mb-6">
                ...
                <div className="z-10 flex-1">
                    <p className="text-lg font-black text-slate-800 tracking-tight leading-snug">
                        Hello, <span className="text-[var(--primary)]">{user.name}</span>! 👋
                    </p>
                    <p className="text-sm text-slate-600 font-medium mt-1">
                        You have <span className="font-black text-[var(--primary)]">{pendingNotes.length} notes</span> to read and <span className="font-black text-purple-600">{pendingMcqs.length} MCQs</span> pending.
                    </p>
                </div>
            </div>
            ...
            <div className="flex p-1 bg-slate-100 rounded-xl mb-6 relative z-10">
                <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${
                        activeFilter === 'TODAY' ? 'bg-white shadow text-slate-800' : 'text-slate-600 hover:text-slate-700'
                    }`}
                >
                ...
                <button
                    ...
                    className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${
                        ['WEAK', 'AVERAGE', 'STRONG', 'EXCELLENT'].includes(activeFilter) ? 'bg-white shadow text-slate-800' : 'text-slate-600 hover:text-slate-700'
                    }`}
                >
                                ...
                                <div>
                                    ...
                                    <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                                        {pendingNotes.map((t, i) => (
                                            <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-100 gap-3 hover:shadow-sm transition-shadow">
                                                <div>
                                                ...
                                                <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-3">
                                                    <div className="text-[10px] font-bold text-[var(--primary)] bg-[var(--primary)]/10 px-2 py-1 rounded-md">
                                                        Revision
                                                    </div>
                                                    <button
                                                        onClick={() => {
                                                        ...
                                                        className="px-4 py-2 bg-[var(--primary)] hover:opacity-90 text-white text-xs font-bold rounded-xl shadow-sm transition-all active:scale-95 flex items-center gap-1.5 shrink-0"
                                                    >
                     ...
                     {completedToday.length > 0 && hubMode === 'PREMIUM' && (
                        <div className="bg-slate-50 rounded-3xl border border-slate-200 p-5 transition-all">
                            ...
                            >
                                ...
                                {showCompletedHistory ? <ChevronUp size={16} className="text-slate-500" /> : <ChevronDown size={16} className="text-slate-500" />}
                            </div>
                                            ...
                                            >
                                                <div className="flex justify-between items-end mb-1.5">
                                                    <div className="flex items-center gap-2 overflow-hidden flex-1">
                                                        ...
                                                        {timer && timer !== 'Ready' && (
                                                            <span className="text-[9px] font-bold text-slate-500 bg-slate-50 px-1.5 py-0.5 rounded flex items-center gap-1 shrink-0 border border-slate-100">
                                                                <Clock size={8} /> {timer}
                                                            </span>
                                                        )}
