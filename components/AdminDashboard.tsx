...
interface ContentConfig {
    ...
    // NEW: Quick Notes
    schoolQuickNotes?: {id: string, title: string, content: string}[];
    competitionQuickNotes?: {id: string, title: string, content: string}[];
    quickNotes?: {id: string, title: string, content: string}[];
...
const AdminDashboardInner: React.FC<Props> = ({ onNavigate, settings, onUpdateSettings, onImpersonate, logActivity, isDarkMode, onToggleDarkMode, user }) => {
  ...
  const [localSettings, setLocalSettings] = useState<SystemSettings>(settings || {
      ...
      termsText: 'By using this app, you agree to our terms. Content is for personal use only. Sharing accounts may lead to a permanent ban.', supportEmail: 'Nadimanwar794@gmail.com', aiModel: 'llama3-8b-8192',
      aiInstruction: '',
  ...
  const handleModeSwitch = (newMode: 'SCHOOL' | 'COMPETITION') => {
      ...
      const currentQuickNotesField = syllabusMode === 'SCHOOL' ? 'schoolQuickNotes' : 'competitionQuickNotes';
      ...
      const updatedConfig = {
          ...
          [currentQuickNotesField]: quickNotes,
      ...
      if (newMode === 'SCHOOL') {
          ...
          // @ts-ignore
          setQuickNotes(updatedConfig.schoolQuickNotes || updatedConfig.quickNotes || []);
      } else {
          // @ts-ignore
          ...
          // @ts-ignore
          setQuickNotes(updatedConfig.competitionQuickNotes || []);
      }
  ...
  const [quickNotes, setQuickNotes] = useState<{id: string, title: string, content: string}[]>([]);
  ...
  const saveChapterContent = async () => {
    ...
    try {
      ...
      const newData = {
          ...
          [syllabusMode === 'SCHOOL' ? 'schoolQuickNotes' : 'competitionQuickNotes']: quickNotes,
  ...
  const applyContentData = (data: any) => {
      ...
      if (syllabusMode === 'SCHOOL') {
          ...
          setQuickNotes(data.schoolQuickNotes || data.quickNotes || []);
      } else {
          ...
          setQuickNotes(data.competitionQuickNotes || []);
      }
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200 animate-in slide-in-from-right">
                      ...
                      {activeTab === 'CONTENT_PDF' && (
                          <div className="space-y-6">
                              ...
                              {/* QUICK NOTES MANAGER */}
                              <div className="bg-yellow-100 p-4 rounded-xl border border-yellow-300 mt-4">
                                  <h4 className="font-bold text-yellow-900 mb-4 flex items-center gap-2">
                                      <Zap size={20} /> Quick Revision Notes (Queek Page)
                                  </h4>
                                  <p className="text-[10px] text-yellow-700 mb-4">
                                      Add dedicated HTML notes for the Quick Revision page. They will appear under the "QUICK" tab.
                                  </p>

                                  <div className="space-y-4 mb-4">
                                      {quickNotes.map((note, idx) => (
                                          <div key={idx} className="bg-white p-4 rounded-xl border border-yellow-200 shadow-sm space-y-3 relative">
                                              <div className="absolute top-2 right-2 flex gap-1">
                                                  <button onClick={() => setQuickNotes(prev => prev.filter((_, i) => i !== idx))} className="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded">
                                                      <Trash2 size={16} />
                                                  </button>
                                              </div>

                                              <div className="flex items-center bg-slate-50 border border-yellow-200 rounded-lg overflow-hidden pr-8">
                                                  <div className="px-3 py-2 text-slate-500 font-bold text-xs">Title</div>
                                                  <input
                                                      type="text"
                                                      value={note.title}
                                                      ...
                                                      placeholder="Note Title (e.g. Formulae)"
                                                      className="flex-1 p-2 bg-transparent text-xs font-bold outline-none text-slate-800"
                                                  />
                                              </div>
                                              ...
                                              <textarea
                                                  ...
                                                  className="w-full p-3 border border-yellow-200 rounded-lg text-xs font-mono h-32 focus:ring-1 focus:ring-yellow-400 outline-none"
                                                  placeholder="<div class='recap'><b>🔁 Recap</b><br>Content here...</div>"
                                              />
                                              <div className="flex justify-end mt-1">
                                                  <button
                                                      onClick={() => {
                                                      ...
                                                      className="text-[10px] font-bold text-yellow-700 bg-yellow-100 px-2 py-1 rounded hover:bg-yellow-200 transition-colors flex items-center gap-1 border border-yellow-300"
                                                  >
                                                      <Zap size={12} /> Add Quick Revision Template
                                                  </button>
                                              </div>
                                          </div>
                                      ))}
                                  ...
                                  <button
                                      onClick={() => setQuickNotes([...quickNotes, { id: Date.now().toString(), title: '', content: '' }])}
                                      className="w-full py-3 bg-white border border-yellow-400 text-yellow-700 font-bold rounded-xl hover:bg-yellow-50 border-dashed flex items-center justify-center gap-2"
                                  >
              ...
              <div className="max-w-2xl space-y-6">
                  ...
                  {activeTab === 'CONFIG_GENERAL' && (
                      <>
                          ...
                          <div><label className="text-xs font-bold uppercase text-slate-600">App Name (Long)</label><input type="text" value={localSettings.appName} onChange={e => setLocalSettings({...localSettings, appName: e.target.value})} className="w-full p-3 border rounded-xl" placeholder="SWG" /></div>
                          ...
                          <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 mt-4">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                  ...
                                  <div className="md:col-span-2 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                                      <label className="text-[10px] font-bold text-slate-600 uppercase block mb-1">Developer Attribution (Developed By)</label>
                                      <input
                                          type="text"
                                          value={localSettings.developedBy || ''}
                                          onChange={e => setLocalSettings({...localSettings, developedBy: e.target.value})}
                                          className="w-full p-2 border rounded-lg text-sm font-bold"
                                          placeholder="Developed by shivangi"
                                      />
                                  </div>
                                  <div className="md:col-span-2 bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
    ...
    <span className="font-bold text-green-600">₹{pkg.price}</span> = {pkg.credits} Credits
                  ...
                  {activeTab === 'CONFIG_EXTERNAL_APPS' && (
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                          ...
                          <p className="text-xs text-slate-600 mb-4">Add up to 4 apps/websites. These will appear in the Student Dashboard.</p>
                              ...
                              {analysisLogs.filter(l => l.userId === viewingUserHistory.id).map(log => (
                                  <div key={log.id} className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm">
                                      <div className="flex justify-between items-start mb-2">
                                          ...
                                          <span className="text-[10px] text-slate-500">{new Date(log.date).toLocaleString()}</span>
                                      </div>
                                  ...
                                  <tbody className="divide-y divide-slate-100">
                                      ...
                                      {(viewingUserHistory.usageHistory || []).map((act: any) => (
                                          <tr key={act.id} className="hover:bg-slate-50">
                                              <td className="p-3 text-slate-600 whitespace-nowrap">{new Date(act.timestamp).toLocaleString()}</td>
                  ...
                  <div className="bg-pink-50 p-4 rounded-xl border border-pink-100 mb-4">
                      ...
                      {giftType === 'SUBSCRIPTION' && (
                          <div className="space-y-2">
                              ...
                              <div className="flex gap-2 items-center">
                                  <label className="text-xs text-slate-600">Duration (Hrs):</label>
                                  <input 
                                      type="number" 
                                      value={giftDuration} 
                  ...
                  {universalNotes.map((note, idx) => (
                      <div key={idx} className={`p-4 rounded-xl border flex justify-between items-center ${note.type === 'HTML' ? 'bg-orange-50 border-orange-100' : 'bg-red-50 border-red-100'}`}>
                          <div>
                              ...
                              {note.type === 'HTML' && <p className="text-xs text-slate-600 mt-1 truncate max-w-md">{note.content.substring(0, 50)}...</p>}
                          </div>
                          <button
                              onClick={() => {
