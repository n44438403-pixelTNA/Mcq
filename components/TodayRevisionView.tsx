...
export const TodayRevisionView: React.FC<Props> = ({ user, topics, onClose, onComplete }) => {
    ...
    useEffect(() => {
        ...
        const loadContent = async () => {
            ...
            // Helper: escape user-supplied strings before injecting into HTML
            ...
            // Render an array of note strings as styled blocks (no explanation, no options).
            const renderNoteBlocks = (notes: string[]): string => notes.map(n => {
                const safe = esc(n)
                    .replace(/\n{2,}/g, '</p><p style="margin:8px 0;color:#334155;font-size:14px;line-height:1.7;">')
                    .replace(/\n/g, '<br/>');
                return `<div style="margin-bottom:12px;padding:12px 14px;background:#f8fafc;border-left:3px solid #6366f1;border-radius:8px;"><p style="margin:0;color:#334155;font-size:14px;line-height:1.7;">${safe}</p></div>`;
            }).join('');

            // Build notes HTML for a SPECIFIC topic from a chapter's wrong MCQs.
            // Show ONLY the `note` field; filter by topic name where possible.
            const buildMistakesHtml = (chapterId: string, topicName: string): string => {
                const history = user.mcqHistory || [];
                const matching = history.filter(h => h.chapterId === chapterId);
                if (matching.length === 0) return '';
                const latest = matching.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
                const wrongs = (latest.wrongQuestions || []) as any[];
                if (wrongs.length === 0) return '';

                const norm = (s: string) => (s || '').toLowerCase().trim();
                const targetTopic = norm(topicName);

                // Prefer wrong Qs whose topic matches the current revision topic.
                let scoped = wrongs.filter(w => {
                    const wt = norm(w.topic);
                    if (!wt || !targetTopic) return false;
                    return wt === targetTopic || wt.includes(targetTopic) || targetTopic.includes(wt);
                });

                // Dedupe note text within the scope.
                const seenNotes = new Set<string>();
                const collect = (arr: any[]): string[] => {
                    const out: string[] = [];
                    for (const w of arr) {
                        const noteRaw = (w.note || '').toString().trim();
                        if (!noteRaw || seenNotes.has(noteRaw)) continue;
                        seenNotes.add(noteRaw);
                        out.push(noteRaw);
                    }
                    return out;
                };

                const notes = collect(scoped);
                if (notes.length === 0) return '';

                return `<div>${renderNoteBlocks(notes)}</div>`;
            };
            ...
            for (const topic of topics) {
                ...
                try {
                    ...
                    // 1) Primary source: notes attached to wrong MCQs from user's MCQ history
                    content = buildMistakesHtml(topic.chapterId);

                    // 2) Fallback: chapter notes from cache/Firestore
                    if (!content || content.length < 50) {
                        ...
                        if (data && Array.isArray(data.topicNotes)) {
            ...
            // Filter: Valid vs Missing — only mark as Self Study if absolutely no usable content
            const valid = loaded.filter(t =>
                t.content &&
                t.content.length > 50 &&
                !t.content.includes("Content not available")
            );
            ...
            {showExitConfirm && (
                <div className="fixed inset-0 z-[100] bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6 text-center">
                         ...
                         <h3 className="text-lg font-black text-slate-800 mb-2">Revision Completed?</h3>
                         <p className="text-slate-600 mb-6 text-sm">
                            Did you finish revising these topics? <br/>
                            <span className="text-xs text-slate-500">(Clicking Yes will move them to MCQ schedule)</span>
                         </p>
                         <div className="flex flex-col gap-2">
                             <button
                                onClick={() => { onComplete(topics); onClose(); }}
                             ...
                             <button
                                ...
                                className="mt-2 text-xs font-bold text-slate-500 hover:text-slate-600"
                             >
                                Cancel (Stay)
                             </button>
                         </div>
